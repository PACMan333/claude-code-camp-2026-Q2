...
- We should have an optional CLI model
- it should manage context and compact messages when reaching out to set limit
...
- we should avoid using Agent SDKs since they already implement features we are implementing by scratch they also might limit our ability to implement exactly what we need.
...
> We did discover that the at somepoint the AI regressed the implementation and Context is still responsible for mangaing tools which is not correct and the tools[] needed to be moved to the Tool Registry
...
> We have to consider the thinking option models, some models have thinking turned on by default where others do not, some cannot turn off thinking. There are other parmeters we can fine tune, but we didn't have much time exploring them in the video.
...
> We use Charm's BubbleTea for the TUI in Ruby, AI thinks bubble Tea is not avaliable for python and so ends up using Texual. In honestly since we have the log_viz we don't really need a TUI but in my original implementation I implemented log_viz later.
