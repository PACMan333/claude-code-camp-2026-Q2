...
class Client:
    ...
    def _read_until(self, request_id):
        while True:
            ...
            if line == "":
                raise self.Error("server closed the connection{}".format(self._stderr_detail()))
            line = line.strip()
            if not line:
                continue
            msg = json.loads(line)
    ...
    # crash during spawn/handshake (bad interpreter version, missing
    # dependency, unhandled exception before the request loop starts) is
    # diagnosable instead of a bare "server closed the connection". stdout
    # hitting EOF means the process is exiting or has exited, so
    # self._process.wait() (bounded -- it's already at EOF) reaps it and
    # guarantees stderr is fully flushed before the read below.
    def _stderr_detail(self):
        try:
            self._process.wait()
            output = self._process.stderr.read()
            return " — stderr: {}".format(output.strip()) if output and output.strip() else ""
